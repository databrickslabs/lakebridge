...
class SamplingQueryBuilder(QueryBuilder):
    ...
    def _get_recon_subquery(self, df: DataFrame) -> exp.Subquery:
        ...
        Emits ``(SELECT 'a' AS c1, ... UNION SELECT ...) AS recon``. Oracle requires
        a ``FROM dual`` on each SELECT; other dialects accept FROM-less SELECTs.
        """
        ...
        needs_dual = get_key_from_dialect(self.engine) == "oracle"
