...
step falls back to a bundled snapshot captured on ``_FALLBACK_FETCHED_AT`` from the same API, so
the extract always carries a price. Writes ``rs_serverless_rpu_prices(region, usd_per_rpu_hour,
fetched_at, source)`` into the profiler DuckDB, where ``source`` is ``'api'`` (``fetched_at`` = run
date) or ``'fallback'`` (``fetched_at`` = snapshot date). Downstream:
...
_FALLBACK_FETCHED_AT = date(2026, 9, 21)
_FALLBACK_PRICES = {
    "us-east-2": 0.36,
    "us-west-2": 0.36,
    "eu-north-1": 0.374,
...
def resolve_region(cred_config_path: str) -> str | None:
    ...
    except (OSError, ValueError, KeyError):  # creds are optional / may be missing or malformed
        return None
    host = config.get("host") if isinstance(config, dict) else None
    if not host:
        return None
    parsed = parse_redshift_endpoint(str(host))
...
def fetch_rpu_price(region: str) -> float | None:
    ...
    The on-demand compute rate is productFamily ``Serverless``, usagetype ending ``:ServerlessUsage``,
    priced per ``RPU-Hr``; reservation rates carry a ``-CR-...`` suffix and are excluded. Returns
    ``None`` if the region has no such SKU, and raises ``ValueError`` if the matching dimensions
    disagree on the price (an unexpected schema change we refuse to guess through).
    """
    response = requests.get(_PRICING_HOST + _REGION_INDEX, timeout=30)
    response.raise_for_status()
    entry = response.json().get("regions", {}).get(region)
    if not entry:
        return None
    response = requests.get(_PRICING_HOST + entry["currentVersionUrl"], timeout=120)
    response.raise_for_status()
    offer = response.json()
    ...
    if not prices:
        return None
    if len(prices) != 1:
        raise ValueError(f"Expected one Serverless RPU price for {region}, found {sorted(prices)}")
    return prices.pop()


def _price_for(region: str) -> tuple[float | None, date | None, str | None]:
    """(price, fetched_at, source): live API price ('api', fetched today) if reachable, else bundled fallback."""
    try:
        price = fetch_rpu_price(region)
        if price is not None:
            return price, date.today(), "api"
    except (requests.RequestException, ValueError, KeyError):  # network/parse failure -> use the bundled fallback
        pass
    fallback = _FALLBACK_PRICES.get(region)
    if fallback is not None:
        return fallback, _FALLBACK_FETCHED_AT, "fallback"
...
def write_prices(db_path: str, cred_config_path: str) -> str:
    ...
    if not region:
        raise RuntimeError("No AWS region resolved from the Redshift endpoint host; cannot price RPU-hours.")
    price, fetched_at, source = _price_for(region)
    if price is None:
    ...
    with connect_to_profiler_db(db_path) as conn:
        conn.execute(
            ...
            "(region VARCHAR, usd_per_rpu_hour DOUBLE, fetched_at DATE, source VARCHAR)"
        )
        conn.execute("INSERT INTO rs_serverless_rpu_prices VALUES (?, ?, ?, ?)", [region, price, fetched_at, source])
    return f"rs_serverless_rpu_prices: {region} = ${price}/RPU-hr ({source}, fetched {fetched_at})"
...
def main() -> None:
    ...
    except (RuntimeError, OSError, duckdb.Error) as e:  # optional step: report and let the pipeline mark ABSENT
