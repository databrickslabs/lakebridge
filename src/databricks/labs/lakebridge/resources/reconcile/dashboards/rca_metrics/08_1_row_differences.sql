/* --title 'Failed Rows' --width 2 --height 3 */
SELECT
    main.recon_id,
    main.source_type,
    ...
    main.start_ts,
    COALESCE(metrics.recon_metrics.column_comparison.absolute_mismatch, 0)
        + COALESCE(metrics.recon_metrics.row_comparison.missing_in_source, 0)
        + COALESCE(metrics.recon_metrics.row_comparison.missing_in_target, 0) AS diff_rows
FROM
