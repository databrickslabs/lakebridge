...
def test_build_query_with_alias_emits_select_with_filter(table_conf, fake_databricks_datasource):
    ...
    assert sql == (
        "SELECT `s_address` AS `s_address`, `s_name` AS `s_name`, `s_suppkey` AS `s_suppkey` "
        "FROM :tbl WHERE `s_suppkey` = 1"
    )
...
def test_build_query_non_databricks_engine_emits_union_recon_cte(table_conf, fake_oracle_datasource):
    df = MagicMock(spec=DataFrame)
