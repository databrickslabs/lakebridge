"""Unit tests for ``orchestrator._fetch_source_rows`` Stage-2 source-fetch behaviour.
...
def _patched_fetch_source_rows(*args, **kwargs):
    """Run ``_fetch_source_rows`` with HashQueryBuilder stubbed to a fixed string."""
    ...
    fake_hash_builder.build_query.return_value = _FAKE_HASH_QUERY
    with patch.object(orch, "HashQueryBuilder", return_value=fake_hash_builder):
        return orch._fetch_source_rows(*args, **kwargs)  # pylint: disable=protected-access
...
def test_fetch_resolves_both_placeholder_forms_for_dialect_parity(source_mock, hash_query_template):
    ...
    fake_hash_builder = MagicMock()
    fake_hash_builder.build_query.return_value = hash_query_template

    ctx = _make_fetch_ctx(source_mock, _make_redshift_query_builder())
    with patch.object(orch, "HashQueryBuilder", return_value=fake_hash_builder):
        orch._fetch_source_rows(  # pylint: disable=protected-access
            ctx,
            solved_hashes={1: [10, 20]},
            unsolved_sb_ids=[],
            report_type="data",
