...
from databricks.labs.lakebridge.reconcile.fingerprint import spark_target
...
def test_query_builder_default_does_not_collapse_empty_to_null():
    """The dispatched builder must default to ``treat_empty_as_null=False`` so the
    fingerprint serialization keeps '' distinct from NULL, matching the row-hash
    convention in expression_generator (TRIM does not collapse '' to NULL).
    """
    builder = get_query_builder("redshift")
    assert builder._treat_empty_as_null is False  # pylint: disable=protected-access
...
def test_resolve_detection_columns_strips_identifier_delimiters():
    ...
    resolved = _resolve_detection_columns(table_conf, src_schema, source)
...
def test_build_mismatch_output_backfills_mismatch_columns_for_report_all(monkeypatch):
    ...
    from databricks.labs.lakebridge.reconcile.recon_output_config import (  # pylint: disable=import-outside-toplevel
        DataReconcileOutput,
        MismatchOutput,
    )
    ...
    class FakeWideDF:
        ...
        def withColumn(self, name, expr_obj):  # pylint: disable=invalid-name  # mirrors PySpark API
    ...
    # pylint: disable=unused-argument  # ``persistence`` accepted to match the real signature.
    ...
    def fake_capture_mismatch_data_and_columns(*, source, target, key_columns):
...
def test_spark_target_uses_decimal_precision_for_hash_aggregates():
    ...
    """
    assert spark_target._RH_OPERAND_TYPE == DecimalType(19, 0)  # pylint: disable=protected-access
    assert spark_target._AGG_TYPE == DecimalType(38, 0)  # pylint: disable=protected-access
