"""B3 regression tests: ``_fetch_source_and_target_rows`` parallel dispatch.
...
def fixture_fetch_ctx():
    """Bare ``_FetchContext`` mock — the parallel helper never inspects it."""
...
def test_fetch_runs_both_workers_concurrently(fetch_ctx):
    ...
    with (
        patch.object(orch, "_fetch_source_rows", side_effect=fake_source),
        patch.object(orch, "_fetch_target_rows", side_effect=fake_target),
    ):
        src_df, fetch_path, tgt_df = orch._fetch_source_and_target_rows(  # pylint: disable=protected-access
...
def test_fetch_returns_serial_equivalent_tuple(fetch_ctx):
    ...
    with (
        patch.object(orch, "_fetch_source_rows", return_value=(fake_src_df, FETCH_PATH_V1_SANDWICH)),
        patch.object(orch, "_fetch_target_rows", return_value=fake_tgt_df),
    ):
        result = orch._fetch_source_and_target_rows(  # pylint: disable=protected-access
            fetch_ctx, solved_hashes={}, unsolved_sb_ids=[1], report_type="data"
        )
...
def test_fetch_reraises_source_failure(fetch_ctx):
    ...
    Pre-B3 a JDBC failure in ``_fetch_source_rows`` would simply re-raise on the
    ...
    with (
        patch.object(orch, "_fetch_source_rows", side_effect=boom),
        patch.object(orch, "_fetch_target_rows", return_value=MagicMock(name="tgt_df")),
    ):
        with pytest.raises(RuntimeError, match="simulated JDBC connection drop"):
            orch._fetch_source_and_target_rows(  # pylint: disable=protected-access
                fetch_ctx, solved_hashes={}, unsolved_sb_ids=[1], report_type="data"
            )
...
def test_fetch_reraises_target_failure(fetch_ctx):
    ...
    with (
        patch.object(orch, "_fetch_source_rows", return_value=(MagicMock(name="src_df"), FETCH_PATH_V1_SANDWICH)),
        patch.object(orch, "_fetch_target_rows", side_effect=boom),
    ):
...
def test_fetch_passes_arguments_through_unmodified(fetch_ctx):
    ...
    with (
        patch.object(orch, "_fetch_source_rows", return_value=(MagicMock(), FETCH_PATH_V1_SANDWICH)) as src_spy,
        patch.object(orch, "_fetch_target_rows", return_value=MagicMock()) as tgt_spy,
    ):
        orch._fetch_source_and_target_rows(fetch_ctx, solved, unsolved, "data")  # pylint: disable=protected-access
...
def test_thread_pool_uses_named_threads_for_observability():
    ...
    src = inspect.getsource(orch._fetch_source_and_target_rows)  # pylint: disable=protected-access
