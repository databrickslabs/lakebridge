...
def test_systemic_mismatch_path_unpersists():
    # Mismatch ratio > 0.15 AND mismatch_count >= the absolute floor -> systemic
    # guard kicks in before the solver runs. (Ratio alone is not enough below the
    # floor — see test_small_table_ratio_alone_does_not_trigger_systemic_guard.)
    src, tgt, joined = _build_chain(total_sbs=300, mismatch_count=60)
...
def test_small_table_ratio_alone_does_not_trigger_systemic_guard():
    """Regression test: a single genuine mismatch on a small table (e.g. the
    6-row sandbox ``diamonds`` fixture: 1/6 ~= 16.7% > 15%) must NOT be treated
    as systemic just because the ratio crosses the threshold. Below the absolute
    ``SYSTEMIC_GUARD_MIN_MISMATCH_COUNT`` floor, the solver path runs normally —
    the systemic guard exists to catch genuinely widespread mismatch, not
    small-sample noise on tables with few populated sub-buckets."""
    src, tgt, joined = _build_chain(total_sbs=6, mismatch_count=1)

    result = detect_and_solve(src, tgt)

    assert result.verdict == "MISMATCH"
    assert result.systemic_mismatch is False
    # Solver path entered: filter().collect() is reached instead of bailing early.
...
def test_source_inspection_documents_single_agg_pattern():
    ...
    assert "_cached_frame(joined)" in body, (
        "joined frame must be wrapped in the _cached_frame(...) context manager so it is "
        "released on every return path (the behavioural test above asserts the actual unpersist call)"
    )
