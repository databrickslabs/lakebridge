...
Covers every branch of ``run_fingerprint_or_reconcile_data``: ineligible / MATCH /
...
from databricks.labs.lakebridge.reconcile.trigger_recon_service import TriggerReconService, pin_utc_session
...
def _config(*, flag: bool = True, source: str = "redshift") -> ReconcileConfig:
    return make_reconcile_config(report_type="data", source=source, reconcile_optimizer=flag)
...
def test_unsupported_dialect_records_unsupported_reason(mock_full):
    ...
    _, metadata = TriggerReconService.run_fingerprint_or_reconcile_data(
        reconciler=_reconciler(),
...
def test_pyspark_exception_during_precheck_falls_back_with_failed_verdict(mock_precheck, mock_full):
    ...
    crash ``do_recon_one``.
    """
    # A plain message is enough — ``PySparkException.__init__`` only requires
    # ``error_class`` when message_parameters/error-class formatting is needed.
    mock_precheck.side_effect = PySparkException("AnalysisException: column 'foo' does not exist on target Delta table")
    mock_full.return_value = _stub_full_pipeline_output()
...
def test_pin_utc_session_sets_utc_when_cluster_is_non_utc():
    """Cross-engine timestamp hashing requires a UTC Spark session. When the cluster
    is on a non-UTC zone, ``pin_utc_session`` must force it to UTC so ``DATE_FORMAT``
    renders the same wall clock the (UTC-pinned) source engine produces."""
    spark = MagicMock()
...
def test_pin_utc_session_is_noop_when_already_utc():
    """Databricks clusters default to UTC — the common case must not touch session
    state (avoids surprising side effects on shared/interactive sessions)."""
    spark = MagicMock()
...
@pytest.mark.parametrize(
    ("source", "flag", "expect_pinned"),
    [
        ("redshift", True, True),
        ("redshift", False, True),  # plain row-hash path needs it too — not gated on the flag
        ("snowflake", True, False),
        ("snowflake", False, False),
    ],
)
def test_create_recon_dependencies_pins_utc_by_dialect_not_by_optimizer_flag(source, flag, expect_pinned):
    """UTC pinning is a row-hash correctness concern tied to the SOURCE DIALECT
    (``_UTC_PINNING_REQUIRED_DIALECTS``), independent of ``reconcile_optimizer``.
    A plain (non-fingerprint) Redshift recon needs it exactly as much as a
    fingerprint-enabled one; a dialect that doesn't need it must never be pinned,
    flag on or off."""
    spark = MagicMock()
    spark.conf.get.return_value = "America/New_York"

    with (
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.verify_workspace_client"),
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.utils.initialise_data_source") as mock_init,
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.Reconciliation"),
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.ReconCapture"),
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.SchemaCompare"),
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.ReconIntermediatePersist"),
        patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.pin_utc_session") as mock_pin,
    ):
        mock_init.return_value = (MagicMock(), MagicMock())
        TriggerReconService.create_recon_dependencies(MagicMock(), spark, _config(flag=flag, source=source))

    assert mock_pin.called is expect_pinned


def test_trigger_recon_restores_original_session_timezone_after_recon():
    """The UTC pin set by create_recon_dependencies must not outlive the recon on
    a shared/interactive cluster: trigger_recon snapshots the pre-recon timezone
    and restores it in the finally block once the recon (and its cleanup) is done."""
    _spark, session_tz = run_timezone_restore_scenario(
        entry_point=TriggerReconService.trigger_recon,
        final_output_target="databricks.labs.lakebridge.reconcile.trigger_recon_service.generate_final_reconcile_output",
        config=_config(flag=False, source="redshift"),
    )
...
@pytest.mark.parametrize(
    "unexpected_exc",
    [
        ValueError("solver produced a degenerate power-sum system"),
        KeyError("sub_bucket_id"),
        IndexError("unsolved_sb_ids out of range"),
        ArithmeticError("overflow in d=2 discriminant"),
        RuntimeError("unanticipated fingerprint fault"),
    ],
)
@patch.object(TriggerReconService, "_run_reconcile_data")
@patch("databricks.labs.lakebridge.reconcile.trigger_recon_service.run_fingerprint_precheck")
def test_unenumerated_precheck_exception_fails_open_not_crash(mock_precheck, mock_full, unexpected_exc):
    """Fail-open contract: an exception type NOT in the enumerated catch list must
    still degrade gracefully to the full pipeline rather than propagate out of
    ``recon_one`` and abort the entire multi-table job. Regression guard for the
    principal-engineer review comment that fail-open was an enumerated allow-list
    rather than a true catch-all boundary.
    """
    mock_precheck.side_effect = unexpected_exc
    mock_full.return_value = _stub_full_pipeline_output()

    # Must NOT raise — the whole point of the catch-all boundary.
    output, metadata = TriggerReconService.run_fingerprint_or_reconcile_data(
        reconciler=_reconciler(),
        reconcile_config=_config(),
        table_conf=_table(),
    ...
    assert metadata.verdict == "FAILED"
    assert metadata.fallback_to_full_pipeline is True
    assert output.mismatch_count == 999, "Unexpected precheck fault must fall back to the full pipeline output"
    mock_full.assert_called_once()
...
def test_pyspark_exception_during_mismatch_output_falls_back_to_full_pipeline(mock_precheck, mock_build, mock_full):
    ...
    mock_build.side_effect = PySparkException("AnalysisException: column resolution failed")
