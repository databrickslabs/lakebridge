...
# Databricks source: clamp to [1, 50_000]; <=0 → 50.
...
def test_databricks_source_keeps_or_caps_sample_value(databricks_normalize_service, table_conf, requested, expected):
    raw = table_conf(sampling_options=SamplingOptions(specifications=_count_spec(requested)))
...
def test_databricks_source_floors_non_positive_sample_value(databricks_normalize_service, table_conf, requested):
    raw = table_conf(sampling_options=SamplingOptions(specifications=_count_spec(requested)))
    result = databricks_normalize_service.normalize_recon_table_config(raw)
    assert result.sampling_options.specifications.value == 50


# Non-Databricks source: kept as-is up to [1, 400]; <=0 → 50; >400 capped with warning.
...
def test_non_databricks_source_keeps_within_400(normalize_service, table_conf, requested, expected):
    raw = table_conf(sampling_options=SamplingOptions(specifications=_count_spec(requested)))
    result = normalize_service.normalize_recon_table_config(raw)
    assert result.sampling_options.specifications.value == expected
...
@pytest.mark.parametrize("requested", [0, -1, -100])
def test_non_databricks_source_floors_non_positive_sample_value(normalize_service, table_conf, requested):
    raw = table_conf(sampling_options=SamplingOptions(specifications=_count_spec(requested)))
    result = normalize_service.normalize_recon_table_config(raw)
    assert result.sampling_options.specifications.value == 50


@pytest.mark.parametrize("requested", [401, 50_001])
def test_non_databricks_source_caps_above_400_with_warning(normalize_service, table_conf, requested, caplog):
    raw = table_conf(sampling_options=SamplingOptions(specifications=_count_spec(requested)))
...
def _count_spec(value):
    return SamplingSpecifications(type=SamplingSpecificationsType.COUNT, value=value)
