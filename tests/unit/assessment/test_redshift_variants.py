...
@pytest.mark.parametrize("platform", _REDSHIFT_PLATFORMS)
def test_redshift_variants_require_connector(platform: str) -> None:
    assert CONNECTOR_REQUIRED[platform] is True
...
@pytest.mark.parametrize("platform", _REDSHIFT_PLATFORMS)
def test_credentials_key_collapses_redshift_variants(platform: str) -> None:
    assert credentials_key(platform) == "redshift"


def test_credentials_key_passes_through_non_redshift_platforms() -> None:
    assert credentials_key("synapse") == "synapse"
    assert credentials_key("mssql") == "mssql"
