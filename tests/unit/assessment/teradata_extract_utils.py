...
def _create_td_user_databases(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.executemany(
        ...
        [
            ("analytics_db", "DBC", datetime(2020, 1, 15), datetime(2024, 6, 1), "F", "NN", 107374182400, 21474836480, 21474836480),
            ("staging_db", "DBC", datetime(2021, 3, 10), datetime(2024, 8, 20), "F", "NN", 53687091200, 10737418240, 10737418240),
        ],
    )
...
def _create_td_pdcr_info_agg_extract(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.execute(
        ...
        [
            "DBQL", date(2025, 3, 1), "Sat", 10,
            datetime(2025, 3, 1, 10, 0), "IT", "Analytics",
            "WD_DEFAULT", "$M$$$", "DEMO_USER",
            "DEMO_USER", "BTEQ", "Select",
            85, 4.79, 0.056, 1.2,
            10127.0, 119.1, 500.0,
            0.0, 0.0, 0.5, 3.2,
        ],
...
def _create_td_dbql_core_info_extract(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.executemany(
        ...
        [
            ("BTEQ", "DEMO_USER", 1001, "SELECT * FROM analytics_db.fact_sales",
             datetime(2025, 3, 1, 10, 0), datetime(2025, 3, 1, 10, 0, 1),
             0.8, 1.2, 5000.0, 1024.0, 7340032.0),
            ("BTEQ", "DEMO_USER", 1002, "SELECT COUNT(*) FROM staging_db.raw_events",
             datetime(2025, 3, 1, 10, 5), datetime(2025, 3, 1, 10, 5, 0),
             0.3, 0.4, 200.0, 128.0, 0.0),
        ],
    )
