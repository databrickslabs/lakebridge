...
# Stand-in for a future fixed-choice source; Redshift no longer registers variants.
_TUPLE_SOURCE = "future_source"
_TUPLE_VARIANTS = {"future_source": ("alpha", "beta")}
...
@pytest.mark.parametrize(("given", "expected"), [("alpha", "alpha"), ("ALPHA", "alpha")])
def test_resolve_variant_explicit_validated_for_tuple_source(given, expected):
    with patch("databricks.labs.lakebridge.assessments.variants.SOURCE_SYSTEM_VARIANTS", _TUPLE_VARIANTS):
        assert resolve_variant(_TUPLE_SOURCE, given) == expected


def test_resolve_variant_explicit_invalid_for_tuple_source_raises():
    with patch("databricks.labs.lakebridge.assessments.variants.SOURCE_SYSTEM_VARIANTS", _TUPLE_VARIANTS):
        with pytest.raises(ValueError, match="Invalid variant"):
            resolve_variant(_TUPLE_SOURCE, "bogus")
...
def test_resolve_variant_tuple_multi_variant_with_auto_raises():
    ...
    with patch("databricks.labs.lakebridge.assessments.variants.SOURCE_SYSTEM_VARIANTS", _TUPLE_VARIANTS):
        with pytest.raises(ValueError, match="Invalid variant"):
            resolve_variant(_TUPLE_SOURCE, AUTO)
