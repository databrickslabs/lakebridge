...
### 1. Environment[​](#1-environment "Direct link to 1. Environment")

* **Lakebridge CLI** installed and configured for your Databricks workspace (same as other profiler sources).

### 2. Network connectivity[​](#2-network-connectivity "Direct link to 2. Network connectivity")
...
### 3. Authentication[​](#3-authentication "Direct link to 3. Authentication")
...
| ----------------------- | ------------------------------------------------------------ |
| **sql\_authentication** | Native database user and password                            |
...
### 4. Database privileges[​](#4-database-privileges "Direct link to 4. Database privileges")

The profiler connects as your configured user and runs read-only extracts. The pipeline includes **source\_ddl** steps that create a helper view **`query_view`** in the database (via `CREATE OR REPLACE VIEW`). The connecting user therefore needs permission to:
...
## Configure Connection to Redshift[​](#configure-connection-to-redshift "Direct link to Configure Connection to Redshift")
