...
| **Input** | Live database connections or query history exports | SQL code files (local filesystem) |
| **Output** | Interactive dashboard with complexity scores and TCO metrics | Per-object complexity report (LOW / MEDIUM / COMPLEX / VERY COMPLEX) |
| **When to use** | Early-stage scoping, executive reporting, sizing estimates | Pre-migration planning, identifying which files need manual review |
...
|---|---|---|---|
| **Approach** | AST-based (ANTLR grammar → IR tree → code gen) | Config/pattern-based converter | LLM-powered (Mosaic AI Model Serving) |
| **Best for** | SQL migration where correctness guarantees matter | ETL platforms + broad SQL dialect coverage | Complex logic, unsupported constructs, or any format via custom prompts |
| **Source dialects** | `mssql` (SQL Server, Azure SQL, RDS for SQL Server), `snowflake` (including dbt repointing), `synapse` (Azure Synapse Analytics dedicated SQL pools) | DataStage, SSIS, Oracle, Teradata, Netezza, Synapse, Redshift, and more | Any SQL dialect or programming language (via built-in or custom YAML prompts) |
| **Output formats** | Databricks SQL only | SQL, SparkSQL, PySpark, notebooks, workflow definitions | Python/SQL notebooks; any text-based format |
...
| **Configuration** | Simple: `source-dialect` flag + I/O paths | Two-layer: LSP params + JSON override files | YAML prompt files; configurable model endpoint |
...
- You are migrating **ETL workloads** (DataStage, SSIS)
- You need **broad SQL dialect coverage** (Oracle, Teradata, Netezza, Redshift, Synapse) with customizable output formats
- You need output as PySpark, SparkSQL notebooks, or workflow definitions — not just Databricks SQL
