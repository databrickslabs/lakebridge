...
| DataStage Connector | System Type | Spark Equivalent | Notes |
|--------------------|-------------|------------------|-------|
| **Oracle Connector** | ORACLE | `spark.read.jdbc()` or SparkSQL | Oracle database |
| **DB2 Connector** | DB2 | `spark.read.jdbc()` or SparkSQL | IBM DB2 database |
| **SQL Server (ODBC)** | MSSQL | `spark.read.jdbc()` or SparkSQL | Microsoft SQL Server |
| **Teradata Connector** | TERADATA | `spark.read.jdbc()` or SparkSQL | Teradata database |
| **Snowflake Connector** | SNOWFLAKE | `spark.read.jdbc()` or SparkSQL | Snowflake cloud DW |
| **Netezza Connector** | NETEZZA | `spark.read.jdbc()` or SparkSQL | Netezza appliance |
| **Hive Connector** | HIVE | SparkSQL | Hive metastore tables |
| **Sequential File** | FLATFILE | `spark.read.csv()` or `csv.\`path\`` | CSV/delimited files |
| **Dataset** | FLATFILE | `spark.read.parquet()` | DataStage dataset files |
| **File Connector** | FLATFILE | `spark.read.csv()` or file reader | Generic file access |
| **Amazon S3** | AMAZON_S3 | `spark.read` with S3 path | AWS S3 storage |
| **HDFS (BDFS)** | FLATFILE | `spark.read` with HDFS path | Hadoop file system |
...
Use the `-u` flag to provide custom configuration overrides:
