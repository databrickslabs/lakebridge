...
This guide covers conversion of Informatica ETL assets to Databricks. BladeBridge supports two Informatica products:

- **PowerCenter** - Informatica's on-premise ETL platform
- **IDMC (Informatica Data Management Cloud)** - Informatica's cloud-based data integration platform (also referred to as InfaCloud)

## Getting Started

### Platform Comparison

| Feature | PowerCenter | IDMC (InfaCloud) |
|---------|-------------|------------------|
| **Output formats** | SparkSQL or PySpark | PySpark only |
| **Conversion scope** | Mappings + Workflows | Mappings only |
| **Input format** | XML export files | ZIP export packages |
| **Invocation** | Lakebridge CLI | dbxconv (requires license key from Lakebridge team)* |

*IDMC conversion via Lakebridge CLI will be available once the LSP server is upgraded to support ZIP file inputs. Note that dbxconv is included with the Lakebridge installation - it is not a separate download.

### Conversion Information

* **Transpiler**: BladeBridge
* **Conversion approach**: Lift and shift - the converter translates Informatica logic to equivalent Databricks code while preserving the original structure and flow
* **Available targets**: Databricks Notebooks (PySpark for both; SparkSQL for PowerCenter only)

---

## PowerCenter Input Requirements
...
## IDMC (InfaCloud) Input Requirements

### Exporting from IDMC

Export your IDMC mappings as ZIP packages from the Informatica Cloud portal:

1. Navigate to your project in IDMC
2. Select the mapping(s) you want to export
3. Use the Export function to create a ZIP package
4. Save the `.zip` file

:::warning Do Not Modify Export Files
The ZIP files exported from IDMC must be preserved exactly as exported. Do not extract, modify, or repackage the ZIP files - the converter requires the exact structure produced by IDMC's export process.
:::

**Export Package Structure:**

When unzipped (for reference only - do not manually unzip for conversion), the export package contains:

```
your-export-package/
├── exportMetadata.v2.json          # Package metadata
├── exportPackage.chksum            # Checksum file
├── ContentsofExportPackage_*.csv   # Contents manifest
├── Explore/
│   ├── Default.Project.json        # Project definition
│   └── Default/
│       └── YourMapping.DTEMPLATE.zip   # Mapping definition (nested ZIP)
└── SYS/
    ├── ConnectionName.Connection.zip    # Connection definitions
    └── AgentGroup.AgentGroup.zip        # Agent configurations
```

**Key Points:**
- IDMC exports are ZIP files containing nested ZIP files
- The converter processes the outer ZIP directly - do not pre-extract
- Connection definitions and agent groups are included in the export
- Only mappings are converted (IDMC task flows are not currently supported by the converter)
...
Informatica objects are converted to their Databricks equivalents as follows:

| Informatica Object | Databricks Equivalent | PowerCenter | IDMC | Description |
|--------------------|----------------------|:-----------:|:----:|-------------|
| **Mapping + Session** | Databricks Notebook | ✓ | - | Combined into one notebook per mapping/session pair (session overrides applied) |
| **Mapping** | Databricks Notebook | - | ✓ | Each IDMC mapping is converted to a PySpark notebook |
| **Workflow** | Databricks Workflow | ✓ | - | Workflows are converted to Databricks Workflow JSON definitions |
| **Worklet** | Databricks Workflow | ✓ | - | Reusable worklets are converted to separate Databricks Workflows |
| **Mapplet** | Shared Python Module | ✓ | ✓ | Reusable mapplets are converted to Python functions in `shared_functions/` |

:::info Mapping and Session Output
In PowerCenter, a Session is a wrapper around a Mapping that contains runtime configuration and potential overrides (SQL statements, connection properties, etc.). The converter combines the mapping logic with any session-level overrides into a single output notebook - there is no separate file for the session. Each mapping/session pair produces one Databricks notebook.
:::
...
The converter generates a synthetic row identifier column called `source_record_id` at the Source Qualifier using the `xxhash64()` hash function. This column is computed from all source columns and is propagated through all downstream transformations, linking the DataFrames together.

:::info Why xxhash64 instead of monotonically_increasing_id?
The PySpark function `monotonically_increasing_id()` cannot be used for row lineage tracking because it re-evaluates every time a DataFrame is accessed. This means the same row could get different IDs across operations, breaking the ability to correlate rows when DataFrames are joined. The `xxhash64()` function produces a deterministic hash based on the actual column values, guaranteeing consistent identification of each row throughout the pipeline.
:::
...
PowerCenter Workflows and Worklets are converted to Databricks Workflow JSON definitions that orchestrate the execution of converted notebooks. The converter replicates the branching logic and execution flow of the original Informatica workflow.

**What Gets Orchestrated:**

| PowerCenter Component | Databricks Equivalent | Description |
|-----------------------|----------------------|-------------|
| **Session** | Notebook Task | Calls a notebook containing the converted mapping logic with session overrides applied |
| **Command Task** | Notebook Task | Shell commands are converted to separate notebooks that execute the command logic |
| **Worklet** | Nested Workflow | Reusable workflow components become separate Databricks Workflows |
| **Decision** | Task Conditions | Conditional branching is preserved using `run_if` conditions |
| **Event Wait/Raise** | Task Dependencies | Event-based coordination is mapped to task dependencies |

**Workflow JSON Structure:**

The converted workflow is a multi-task Databricks Workflow that preserves:

- **Task Dependencies**: The `depends_on` array captures which tasks must complete before others start
- **Execution Order**: Tasks execute in the same sequence as the original workflow
- **Branching Logic**: Parallel paths and conditional execution are maintained
- **Failure Handling**: Email notifications and retry logic are configured per task
...
```json
{
  ...
  "tasks": [
    {
      "task_key": "wf_customer_load_parameters",
      "notebook_task": {
        "notebook_path": "/Workspace/Users/wf_customer_load_params.py"
      },
      "run_if": "ALL_SUCCESS"
    },
    {
      "task_key": "s_m_customer_extract",
      "depends_on": [
        { "task_key": "wf_customer_load_parameters" }
      ],
      "notebook_task": {
        "notebook_path": "/Workspace/Users/m_customer_extract",
        "source": "WORKSPACE"
      },
      "max_retries": 3,
      "run_if": "ALL_SUCCESS"
    ...
    {
      "task_key": "s_m_customer_transform",
      "depends_on": [
        { "task_key": "s_m_customer_extract" }
      ],
      "notebook_task": {
        "notebook_path": "/Workspace/Users/m_customer_transform",
        "source": "WORKSPACE"
      },
      "run_if": "ALL_SUCCESS"
    }
  ...
  "email_notifications": {
    "on_failure": ["etl-alerts@company.com"],
    "on_success": ["etl-alerts@company.com"]
  }
}
```
...
- Each session becomes a notebook task that calls the notebook containing the actual mapping logic
- Session-level overrides (SQL statements, connection properties) are incorporated into the notebook code
- A parameters notebook is generated to initialize workflow variables at the start
- Command tasks (shell commands) become separate notebook tasks
- The workflow preserves the original Informatica task names for easy correlation with source objects
...
BladeBridge uses a layered configuration system where configs inherit from base configs. Based on the target technology selected, the converter loads the appropriate configuration hierarchy.

**SparkSQL Output - Inheritance Path:**

```
base_infapc2databricks_sparksql.json
    └── inherits from: base_infapc2databricks.json
            └── inherits from: base_etl2databricks_defs.json
```

**PySpark Output - Inheritance Path:**

```
base_infapc2databricks_pyspark.json
    └── inherits from: base_infapc2databricks.json
            └── inherits from: base_etl2databricks_defs.json
```

**Expression Conversion Config Files:**

Each main config file references additional configs for expression and function translation:

| Setting | SparkSQL Config | PySpark Config | Purpose |
|---------|-----------------|----------------|---------|
| `etl_converter_config_file` | `infa2databricks.json` | `base_infapc2pyspark.json` | Converts ETL-level expressions (PowerCenter functions like IIF, DECODE, date functions) |
| `expr_sql_converter_config_file` | (not used) | `base_infa2databricks_expr.json` | PySpark only - complex expressions containing IIF or DECODE are wrapped in `expr('...')` construct |

**Embedded SQL Conversion:**

When PowerCenter mappings contain embedded SQL (e.g., in Source Qualifier SQL Override or Lookup SQL), the converter identifies the source database type and applies the appropriate SQL dialect translation rules from the tech mapper configuration.

**Database Type Determination:**

The source database type is determined from the session-level connection settings, not from the source/target definition at the folder level. Specifically, the converter reads the `CONNECTIONSUBTYPE` attribute from the `CONNECTIONREFERENCE` element within the `SESSIONEXTENSION`:

```xml
<SESSIONEXTENSION NAME="Relational Reader" SINSTANCENAME="SQ_CUSTOMERS" ...>
  <CONNECTIONREFERENCE 
    CNXREFNAME="DB Connection" 
    CONNECTIONNAME="Oracle_Prod" 
    CONNECTIONSUBTYPE="Oracle"           <!-- This determines the SQL dialect -->
    CONNECTIONTYPE="Relational" />
</SESSIONEXTENSION>
```

This session-level connection information takes precedence and enables the converter to apply the correct SQL translation rules (e.g., Oracle to Spark, Teradata to Spark, SQL Server to Spark) for any embedded SQL within that source or lookup.
...
1. **Session-level Connection** (primary source): The `CONNECTIONSUBTYPE` from the session's `CONNECTIONREFERENCE` element determines the database type. This takes precedence because it reflects the actual runtime connection:
```xml
<CONNECTIONREFERENCE CONNECTIONSUBTYPE="Oracle" CONNECTIONTYPE="Relational" ...>
```

2. **Source/Target Definition** (fallback): If session connection info is not available, the `DATABASETYPE` attribute from the source/target definition is used:
```xml
<SOURCE DATABASETYPE="Oracle" NAME="CUSTOMERS" ...>
```

3. **`system_type_class`** in config maps database types to categories:
```json
"system_type_class": {
  "ORACLE": "RELATIONAL",
  "MySQL": "RELATIONAL",
...
4. **`commands`** section defines reader/writer templates per category:
```json
"commands": {
...
**Dispatch Flow:**
```
Session CONNECTIONSUBTYPE (or fallback: XML DATABASETYPE) → system_type_class mapping → READER/WRITER command template
```

### Column Conforming

The converter generates column conforming code to ensure source columns match expected component layouts. This handles database case sensitivity differences and missing aliases between source definitions and transformation port names.
...
### PowerCenter Conversion

1. **Export PowerCenter mappings** to PowerMart XML format (include sessions and workflows)
...
### IDMC (InfaCloud) Conversion

1. **Export IDMC mappings** as ZIP packages from the Informatica Cloud portal
2. **Run conversion** using dbxconv (contact Lakebridge team for license key):
   ```bash
   dbxconv --input /path/to/export.zip \
     --output /output/pyspark \
     --config infacloud_config.json
   ```
...
7. **Deploy workflows** to production (PowerCenter only)
...
#### Incomplete Export - Missing Sessions or Workflows

**Problem:** The converter produces mapping notebooks but no workflow JSON files.

**Cause:** The XML export contains only mappings without the associated sessions and workflows. This commonly happens when exporting from PowerCenter Designer instead of Repository Manager, or when only mapping objects are selected during export.

**Solution:** Re-export from Repository Manager and ensure you select the complete folder or explicitly include:
- All mappings
- All sessions that reference the mappings
- All workflows that orchestrate the sessions

#### Export from Workflow Manager Missing Dependencies

**Problem:** Conversion errors or missing transformations when using exports from Workflow Manager.

**Cause:** Exports created directly from Workflow Manager may not include all dependent objects (sources, targets, shared objects, shortcuts).

**Solution:** Always export from Repository Manager using the "Export Objects" wizard, which properly resolves and includes all dependencies.

### IDMC (InfaCloud) Issues

#### Path Too Long Error (Windows)

**Problem:** Conversion fails with path-related errors, especially on Windows.

**Cause:** IDMC export ZIP files contain deeply nested folder structures. When the converter extracts these files, the combined path length can exceed Windows' 260-character (or 1024-character extended) path limit.

**Solution:** 
- Use shorter paths for both input and output folders
- Avoid spaces in folder and file names
- Place the input ZIP file close to the drive root (e.g., `C:\infa\input.zip`)
- Use a short output path (e.g., `C:\infa\out`)
- On Windows, consider enabling long path support in Group Policy

#### Modified ZIP File Errors

**Problem:** Converter fails to parse the IDMC export or produces unexpected errors.

**Cause:** The ZIP file was manually extracted, modified, or repackaged. The converter expects the exact structure produced by IDMC's export process.

**Solution:** Use a fresh, unmodified ZIP file directly from IDMC export. Do not:
- Extract and re-zip the contents
- Rename files within the ZIP
- Add or remove files from the package
- Use third-party ZIP tools to "fix" the archive

### General Issues

#### Output Folder Permission Errors

**Problem:** Conversion fails when writing output files.

**Cause:** The output folder doesn't exist or lacks write permissions.

**Solution:**
- Ensure the output folder exists before running the conversion
- Verify you have write permissions to the output location
- On shared drives, check network permissions

#### Unsupported Transformation Warnings

**Problem:** Conversion completes but with warnings about unsupported transformations.

**Cause:** Some transformations (Java, External Procedure, Custom Transformation) cannot be automatically converted.

**Solution:** These warnings are expected. Review the generated notebooks and:
- Implement custom logic manually in Python
- Use the warnings to identify the percentage of code requiring manual attention
- Consider creating post-conversion automation for recurring patterns
