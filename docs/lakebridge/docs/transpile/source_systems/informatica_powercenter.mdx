...
When PowerCenter mappings contain embedded SQL (e.g., in Source Qualifier SQL Override or Lookup SQL), the converter identifies the source database type from the connection and applies the appropriate SQL dialect translation rules from the tech mapper configuration.
