...
# SSIS Package Variables
V_ProcessDate = f'2024-01-01'
V_PackageName = f'LoadCustomerData'

# COMMAND ----------

# Processing node Package\Startup Tasks\Truncate Staging Table, type EXECUTE_SQL
...
# Processing node Package\ETL_Customers\OLE DB Source, type SOURCE
# component nameOLE DB Source
Package_ETL_Customers_OLE_DB_Source = spark.read \
    .format("jdbc") \
    .option("url", "source-db-url") \
    .option("dbtable", "dbo.Customers") \
    .option("user", "source-db-user") \
    .option("password", "source-db-password") \
    .load()
Package_ETL_Customers_OLE_DB_Source.createOrReplaceTempView('Package_ETL_Customers_OLE_DB_Source')

# COMMAND ----------

# Processing node Package\ETL_Customers\Derived Column, type DERIVED_COLUMN
# component nameDerived Column
Package_ETL_Customers_Derived_Column = f"""
SELECT 
    *,
    CONCAT(FirstName, ' ', LastName) AS FullName,
    CURRENT_TIMESTAMP() AS ProcessDate
FROM Package_ETL_Customers_OLE_DB_Source
"""
Package_ETL_Customers_Derived_Column = spark.sql(Package_ETL_Customers_Derived_Column)
Package_ETL_Customers_Derived_Column.createOrReplaceTempView('Package_ETL_Customers_Derived_Column')

# COMMAND ----------

# Processing node Package\ETL_Customers\Lookup Customer Type, type LOOKUP
# component nameLookup Customer Type
Package_ETL_Customers_Lookup = f"""
SELECT 
    src.*,
    ref.TypeName,
    ref.TypeDescription
FROM Package_ETL_Customers_Derived_Column src
LEFT JOIN dim.customer_type ref
    ON src.CustomerTypeID = ref.TypeID
"""
Package_ETL_Customers_Lookup = spark.sql(Package_ETL_Customers_Lookup)
Package_ETL_Customers_Lookup.createOrReplaceTempView('Package_ETL_Customers_Lookup')

# COMMAND ----------

# Processing node Package\ETL_Customers\Conditional Split, type CONDITIONAL_SPLIT
# component nameConditional Split - Active Customers
Package_ETL_Customers_Conditional_Split_Active = f"""
SELECT * 
FROM Package_ETL_Customers_Lookup
WHERE Status = 'ACTIVE'
"""
Package_ETL_Customers_Conditional_Split_Active = spark.sql(Package_ETL_Customers_Conditional_Split_Active)
Package_ETL_Customers_Conditional_Split_Active.createOrReplaceTempView('Package_ETL_Customers_Conditional_Split_Active')

# COMMAND ----------

# Processing node Package\ETL_Customers\OLE DB Destination, type DESTINATION
# component nameOLE DB Destination
Package_ETL_Customers_OLE_DB_Destination = f"""
...
SELECT 
    ...
    TypeName,
    ProcessDate
FROM Package_ETL_Customers_Conditional_Split_Active
"""
spark.sql(Package_ETL_Customers_OLE_DB_Destination)

# COMMAND ----------

# Processing node Package\Completion Tasks\Update Control Table, type EXECUTE_SQL
