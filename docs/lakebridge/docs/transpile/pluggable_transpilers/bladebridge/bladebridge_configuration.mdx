...
|---|---|---|
| **Layer 1 — LSP parameters** | Source dialect, target output format, override file path | Set via `install-transpile` prompts or `transpile` CLI flags |
...
### Dialect-specific `target-technology` constraints
...
| `informatica cloud` | No | Defaults to `SQL`; no prompt offered |
