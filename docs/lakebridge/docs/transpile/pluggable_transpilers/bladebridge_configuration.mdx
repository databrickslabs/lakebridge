...
SQL configuration files control the conversion of SQL code and statements.  
They define the rules, patterns, and templates used to translate source SQL dialects (e.g., Oracle, SQL Server, Teradata) into target Databricks SQL syntax.

Key sections in SQL configuration files include:

- **inherit_from** – Inherit base configuration files for the chosen source system.  
- **target_sql_file_header** – Add catalog or schema-level statements to the beginning of the generated SQL output.  
- **line_subst** – Perform simple regex-based substitutions within single lines (e.g., schema replacements).  
- **block_subst** – Perform multi-line or structural substitutions.  
- **function_subst** – Define conversions for SQL functions.  

### Example Section

In this example, we will **modify the schema and catalog names** used during conversion.

We need to add inheritance in our custom configuration file like this:
