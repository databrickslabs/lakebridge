...
- **Serverless job compute** (default) or **Classic job compute** (optional)
  - During installation, you'll be prompted to choose between serverless (default) and classic job compute
...
```
Use serverless compute for Switch job? (no = use classic job compute instead) (default: yes):
```
- **yes (default)**: Uses serverless job compute
- **no**: Uses classic job compute with LTS Spark version and USER_ISOLATION mode

The installation automatically:

1. **Uploads Notebooks**: Switch processing notebooks to workspace
2. **Creates Databricks Job**: Configured job with your selected compute type
