...
databricks labs lakebridge llm-transpile \
  ...
  [--volume your_volume] \
  [--foundational-model your_foundation_model] \
...
| `--foundational-model` | Optional (prompted from available FM APIs) | Model serving endpoint name for conversions | `databricks-claude-sonnet-4-5` |
