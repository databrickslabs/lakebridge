...
4. **Foundation Model**: Model serving endpoint (e.g., `my_foundation_model`)
...
The `llm-transpile` command accepts the following parameters. Parameters without default values must be provided as CLI arguments. Parameters with default values will be prompted interactively if not provided via CLI.
...
| `--catalog-name` | Unity Catalog for Switch Delta tables and Volume | `lakebridge` | `my_catalog` |
| `--schema-name` | Schema within the catalog for Switch Delta tables and Volume | `switch` | `my_schema` |
| `--volume` | Unity Catalog Volume for uploaded input source files | `switch_volume` | `my_volume` |
| `--foundational-model` | Model serving endpoint name for conversions | (selected from available FM APIs in workspace) | `databricks-claude-sonnet-4-5` |
...
Convert Snowflake SQL to Databricks Python notebooks using the built-in Snowflake conversion prompt. Foundation model will be prompted interactively:
