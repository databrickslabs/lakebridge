...
Override the row-hash function with your own SQL. Each
value is raw SQL containing a single `{}` placeholder that Lakebridge replaces with the
concatenated hash input.

- `source` defaults to `None`. When omitted, the source side falls back to the dialect default.
- `target` defaults to `sha2({}, 256)`.

For Teradata sources, `source` is **mandatory** since Teradata has no native cryptographic hash.
