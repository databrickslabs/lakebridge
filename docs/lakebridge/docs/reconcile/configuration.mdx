...
<Tabs>
    ...
    <TabItem value="teradata" label="Teradata">
        ...
        :::warning Required: install a SHA-256 UDF on the Teradata source
        Teradata does not provide a native cryptographic hash that yields a hex string in pure
        SQL. The Teradata source-side hash therefore defaults to a **user-installed** UDF named
        `lakebridge_sha256_hex(VARCHAR) RETURNS VARCHAR(64)` that produces the lowercase SHA-256
        hex digest, matching `SHA2(<concat>, 256)` on the Databricks side. You are responsible
        for building, installing, and registering this UDF.

        Skeleton of the expected signature (the `RETURN` body is a placeholder; substitute your
        actual SHA-256 hex implementation):

        ```sql
        REPLACE FUNCTION <source_database>.lakebridge_sha256_hex(
            input_text VARCHAR(32000) CHARACTER SET UNICODE
        )
        RETURNS VARCHAR(64)
        LANGUAGE SQL
        CONTAINS SQL
        DETERMINISTIC
        RETURN '<replace with sha256 hex of input_text>';
        ```

        Refer to the Teradata documentation for how to create and register UDFs on your release
        and platform.

        Install this UDF in the source database before running any `row`, `data`, or `all` report
        (the `schema` report type does not need it). To point at a UDF installed under a different
        name or schema, set `hash_expression` on the `source:` / `target:` connection block; see
...
The Teradata default expects a UDF with the following contract, callable from the source
database that the reconcile query targets:

| | |
|---|---|
| **Name** | `lakebridge_sha256_hex` |
| **Argument** | a single `VARCHAR` input (sized to fit the longest concatenated row-key — `VARCHAR(32000) CHARACTER SET UNICODE` is a safe default) |
| **Returns** | `VARCHAR(64)` — lowercase hex digest of SHA-256 over the input |

If your environment already exposes a SHA UDF under a different name or schema, leave this
default alone and point Lakebridge at it on the source/target connection config via
[`hash_expression`](#hash-expression).
...
Override the dialect's default row-hash function with your own SQL. Configure it on the
**connection** (`source:` / `target:`). The hash function is a property of the source/target
system, not of any individual table. Each value is raw SQL containing a single `{}` placeholder
that Lakebridge replaces with the concatenated hash input.
...
<Tabs>
  <TabItem value="python" label="Python" default>
    ...
    class SourceConnectionConfig:
        ...
        hash_expression: str | None = None  # raw SQL with a single `{}` placeholder
...
If `hash_expression` is omitted on either side, that side falls back to the dialect default.
