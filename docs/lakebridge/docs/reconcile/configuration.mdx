...
| `value` | int (float for fraction) | Maximum rows pulled into each sample bucket. | `50` |

**Engine-aware bounds on `value`:**

- **Databricks source:** clamped to `[1, 50_000]`. Larger values are silently capped at 50,000.
- **Non-Databricks source:** clamped to `[1, 50]`. Larger values are capped at 50 with a `WARNING` log (`only honored for Databricks source; capping to 50 for <connector>`).
...
Random sampling — cap each bucket at 500 rows:
```json
"sampling_options": {
  "method": "random",
...
Stratified sampling — 10 buckets keyed by `ss_store_sk`, 500 rows per bucket:
```json
"sampling_options": {
  "method": "stratified",
  "specifications": { "type": "count", "value": 500 },
