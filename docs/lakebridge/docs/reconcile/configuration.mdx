...
- **Databricks source:** clamped to `[1, 50_000]`. Larger values are silently capped at 50,000.
- **Non-Databricks source:** honored up to `400`. Larger values are capped at 400 with a `WARNING` log (`exceeds the non-Databricks limit of 400; capping to 400 for <connector>`).
