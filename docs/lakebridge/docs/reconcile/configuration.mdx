...
- The pre-check selects a sub-bucket / bucket count adaptively from the target's
  ...
  is missing, it falls back to a static `(1 048 576, 32 768)` pair.
...
When you provide a transformation expression, Reconcile uses it as-is. **You must handle nulls and cast to string explicitly** in the expression.
Use `coalesce(cast(... as string), '_null_recon_')` to avoid null mismatches and incompatibility with the internal transformations that rely on strings.
:::
