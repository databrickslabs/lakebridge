...
<Tabs>
    ...
    <TabItem value="teradata" label="Teradata">
        ...
        Teradata ships no portable cryptographic hash that produces a hex string in pure SQL. The
        Teradata source-side hash therefore defaults to a **user-installed** UDF named
        `lakebridge_sha256_hex(VARCHAR) RETURNS VARCHAR(64)` that produces the lowercase SHA-256
        hex digest — matching `SHA2(<concat>, 256)` on the Databricks side.

        Install this UDF in the source database before running any `row`, `data`, or `all` report
        (the `schema` report type does not need it). To point at a different UDF, override
        per-table via [`hash_expression`](#hash-expression).
        :::
    </TabItem>
...
default alone and point Lakebridge at it per-table via [`hash_expression`](#hash-expression).
...
| `hash_expression` | HashExpression | No | Override the dialect's default row-hash function (see [Hash Expression](#hash-expression)). Required for Teradata if no `lakebridge_sha256_hex` UDF is installed. |
...
Override the dialect's default row-hash function with your own SQL. Provide the source-side
expression, the target-side expression, or both — each is raw SQL containing a single `{}`
...
<Tabs>
  <TabItem value="python" label="Python" default>
    ...
    class HashExpression:
        source: str | None = None  # SQL for the source-side hash; must contain a single `{}`
        target: str | None = None  # SQL for the target-side hash; must contain a single `{}`
    ```
  </TabItem>
  <TabItem value="json" label="JSON">
    ```json
...
If either `source` or `target` is omitted, that side falls back to the dialect default.
