...
[^teradata-hash]: Teradata ships no portable SHA in pure SQL, so row-hash report types (`row`, `data`, `all`) require a user-installed SHA-256 UDF on the source — see [Teradata Hash UDF](/docs/reconcile/configuration#teradata-hash-udf).
