...
[^bigquery-types]: A few BigQuery types have no exact Databricks equivalent and are reconciled against their standard migration target (`BIGNUMERIC`→`STRING`, `TIME`→`STRING`, `JSON`→`VARIANT`, `RANGE<T>`→`STRUCT<start, end>`, `DATETIME`→`TIMESTAMP_NTZ`). If your migration chose a different target the `schema` report flags it for review. `INTERVAL` migrates to two Databricks columns and is not supported by `schema` reconciliation.
