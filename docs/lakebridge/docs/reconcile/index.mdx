...
[^bigquery-types]: Two groups of BigQuery types cannot be compared exactly, and `schema` logs a warning naming the affected columns. Types with no equivalent in Databricks — `BIGNUMERIC`, `TIME`, `RANGE<T>`, nested `JSON` such as `ARRAY<JSON>`, or these nested inside `ARRAY`/`STRUCT` — are left as their original BigQuery type and may be reported as mismatches even when the data migrated correctly. Conversely, some lossy targets are reported as a match: every BigQuery integer type is an alias of `INT64` and both float types of `FLOAT64`, so the declared width is not recoverable and a narrower Databricks target (`int`, `smallint`, `float`) compares as equal even though it cannot hold the full source range; likewise a `DATETIME` column matches a `timestamp` target, which reads wall-clock values as instants. Verify these manually.
