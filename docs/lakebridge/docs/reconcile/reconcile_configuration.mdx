...
| `max_sample_size`       | `int`                    | Per-table cap on sample rows persisted to the `details` table for each recon category — mismatch, missing_in_source, missing_in_target, threshold (see [max_sample_size](#max-sample-size)) | Optional    | <CodeBlock language="json">{`{"max_sample_size": 200}`}</CodeBlock>                                                                                     |
...
Per-table cap on how many sample rows are persisted to the `details` table for each recon category — `mismatch`, `missing_in_source`, `missing_in_target`, and `threshold_mismatch`. The recon `metrics` table always stores the **true total counts**; this knob only controls how many example rows are kept for drill-down.
...
| Input                                           | Result                                                            |
|-------------------------------------------------|-------------------------------------------------------------------|
| Not provided / `null`                           | Defaults to `50`                                                  |
| Integer in `[50, 50000]`                        | Used as-is                                                        |
| Integer below `50` (incl. `0`, negative values) | Floored to `50`; an `INFO` log line is emitted                    |
| Integer above `50000`                           | Capped to `50000`; an `INFO` log line is emitted                  |
| Boolean (`true` / `false`)                      | Raises `InvalidMaxSampleSizeException` at config load             |
| String, float, or any non-int                   | Raises `InvalidMaxSampleSizeException` at config load             |
