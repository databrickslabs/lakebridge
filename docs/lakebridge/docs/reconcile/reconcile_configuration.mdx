...
The config file must be named as `recon_config_[DATA_SOURCE]_[SOURCE_UNITY_CATALOG_CONNECTION_NAME]_[REPORT_TYPE].json` and should be placed in the Lakebridge root directory `.lakebridge` within the Databricks Workspace.

> The filename pattern would remain the same for all the data_sources.

``` shell
recon_config_${DATA_SOURCE}_${SOURCE_UNITY_CATALOG_CONNECTION_NAME}_${REPORT_TYPE}.json
```
...
<Tabs>
    <TabItem value="snowflake" label="Snowflake" default>
        ``` yaml title="recon_config_snowflake_example_connection_snowflake_all.json"
        source:
          dialect: snowflake
          catalog: sample_data
          schema: default
    ...
    <TabItem value="oracle" label="Oracle">
        ``` yaml title="recon_config_oracle_example_connection_oracle_data.json"
        source:
          dialect: oracle
    ...
    <TabItem value="tsql" label="MS SQL Server (incl. Synapse)">
        ``` yaml title="recon_config_tsql_example_connection_mssql_data.json"
        database_config:
          dialect: mssql
...
> **Note:** the filename must be created in the same case as `uc_connection_name` is defined. Or `catalog` for databricks sources
> For example, if the source connection name is defined as `ORC_connection` in the `reconcile` config, the filename should be `recon_config_oracle_ORC_connection_data.json`.
...
| `jdbc_reader_options`   | `object`                 | JDBC read parallelization configuration                                        | Optional    | <CodeBlock language="json">{`{"jdbc_reader_options": {"num_partitions": 10, "lower_bound": : 1, upper_bound: 99, "partition_column": "id"}}`}</CodeBlock>|
...
| `sampling_options`      | `SamplingOptions`        | Caps the number of rows pulled into each sample bucket (mismatch / missing-in-src / missing-in-tgt). See [Sampling Options](#sampling-options) | Optional    | <CodeBlock language="json">{`{"sampling_options": {"method": "random", "specifications": {"type": "count", "value": 50}}}`}</CodeBlock>              |
...
<Tabs>
  ...
  <TabItem value="json" label="JSON">
    ```json
      "jdbc_reader_options": {
          "num_partitions": "<NUMBER_PARTITIONS>",
...
|-------------------|-----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------|---------------|
| num_partitions    | string    | the number of partitions for reading input data in parallel                                                                                                                                                                                                                                                                                                                                                                                                             | required                | "200"         |
...
For `data` and `all` report types, lakebridge captures sample rows for each
mismatch / missing bucket. `sampling_options` controls *how many* and *how*
samples are drawn.
...
|---------|---------------------------------|-------------------------------------------------------------------------------------------------------------------|---------|
| `type`  | enum (`count` \| `fraction`)    | `count` = absolute row cap. `fraction` is reserved and currently disabled — passing it raises a config error.     | `count` |
| `value` | `int` (or `float` for fraction) | Maximum rows pulled into each sample bucket.                                                                      | `50`    |

:::note Engine-aware bounds on `value`
- **Databricks source:** `value` clamped to `[1, 50_000]`. Larger values silently capped at 50_000.
- **Non-Databricks source:** `value` clamped to `[1, 50]`. Larger values capped at 50 with a WARNING log: `only honored for Databricks source; capping to 50 for <connector>`.
- `value <= 0` floors to 50.
:::

### SamplingOptions

Wraps `SamplingSpecifications` and adds the sampling method.
...
| field                | type                            | description                                                                                                  | default              |
|----------------------|---------------------------------|--------------------------------------------------------------------------------------------------------------|----------------------|
| `method`             | enum (`random` \| `stratified`) | Sampling strategy.                                                                                           | `random`             |
| `specifications`     | `SamplingSpecifications`        | Size policy (see [above](#samplingspecifications)).                                                          | defaults to count/50 |
...
When `method = stratified`, rows are bucketed by `pmod(abs(hash(stratified_columns)), stratified_buckets)` and sampling proceeds within each bucket. If `stratified_columns` or `stratified_buckets` is missing, init raises a `ValueError`.
...
```json
{
  "tables": [{
    ...
    "target_name": "store_sales",
    "join_columns": ["ss_item_sk", "ss_ticket_number"],
    "sampling_options": {
      "method": "random",
...
Each sample bucket (mismatch / missing-in-src / missing-in-tgt) caps at 500 rows. If `sampling_options` is omitted entirely, `value` defaults to 50.
