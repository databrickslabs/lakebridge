...
- `data_source`: The data source to be reconciled. Supported values: `snowflake`, `oracle`, `mssql`, `synapse`, `databricks`, `redshift`.
...
We use the class `TableRecon` to configure the tables to be reconciled.
...
:::note
Schema and catalog information are configured using `DatabaseConfig` inside `ReconcileConfig` (see above).
:::
