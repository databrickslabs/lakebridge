...
The auto-configure module exposes two functions over a `TableRecon`:

- `discover_tables(...)` — list source/target schemas, save the matched table pairs.
- `auto_configure_tables(table_recon, ...)` — apply all registered configurers (column mapping today; join keys, transformations, ... as they're added) to each Table in `table_recon` and save the result.

Both take your reconcile config and a Spark session, and build the source/target adapters internally:
...
discovered = discover_tables(installation=installation, reconcile_config=reconcile_config, spark=spark)
table_recon = auto_configure_tables(discovered, installation=installation, reconcile_config=reconcile_config, spark=spark)

for t in table_recon.tables:
    print(t.source_name, "→", t.target_name)
```

Both functions upload the resulting `TableRecon` to the install folder as `reconcile_config.table_recon_filename`.
...
1. **Discover** — call `discover_tables(...)`. This writes a `TableRecon` to the install folder containing only matched source/target table pairs (no `column_mapping`).
...
3. **Auto-configure** — load the curated file and call `auto_configure_tables(curated, ...)`.
...
auto_configure_tables(curated, installation=installation, reconcile_config=reconcile_config, spark=spark)
```
...
To run all supported configurers against just one table — e.g. after a column was added on the source — call `auto_configure_table`. It returns a configured `Table` without touching the file in the install folder:
