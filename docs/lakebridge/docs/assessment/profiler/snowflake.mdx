...
Per-query credits come from `SNOWFLAKE.ACCOUNT_USAGE.QUERY_ATTRIBUTION_HISTORY` where available, falling back to an execution-time × warehouse-size-rate estimate (flagged by `IS_CREDITS_ESTIMATED`).
