...
- The PAT itself (paste when prompted for password)
...
The extract is written to a DuckDB file (`profiler_extract.db`) in the configured output directory. It contains raw `SNOWFLAKE.ACCOUNT_USAGE` data — warehouse usage, query history, storage, users, and (when applicable) pipe / autoclustering / materialized-view refresh credits. Inspect it with:
