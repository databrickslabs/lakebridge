...
:::warning Attention:
Object-definition extracts (`table_definitions`, `column_definitions`, `routine_definitions`) include recreation DDL and routine bodies. Treat the DuckDB output as sensitive — the same way you would treat exported object scripts. Clone/snapshot `base_table_*` and materialized-view `replica_source_*` columns capture **source structural provenance** (dependency links), not Unity Catalog runtime lineage.
:::
...
The pipeline runs `bq_metadata_extract` — connects to BigQuery and runs 19 vendored SQL queries serially across the configured `(project, region)` pairs and in parallel within each iteration (`max_parallel_sqls` workers). Results land in `~/.databricks/labs/lakebridge_profilers/bigquery_assessment/profiler_extract.db` (DuckDB).

The final DuckDB contains **15 analysis tables** by default (one per `analysis_type`): 12 sizing/workload tables plus `table_definitions`, `column_definitions`, and `routine_definitions`.
