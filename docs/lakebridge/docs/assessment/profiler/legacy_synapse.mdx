...
`ActiveDirectoryInteractive` is the right choice when Conditional Access requires MFA. `ActiveDirectoryServicePrincipal` is the right choice for headless / CI runs against an Entra ID-enabled dedicated SQL pool.
