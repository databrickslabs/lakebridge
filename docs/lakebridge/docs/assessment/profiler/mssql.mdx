...
| `ActiveDirectoryDefault` | Entra ID via the `DefaultAzureCredential` chain — `az login`, SPN env vars, or managed identity. Recommended for Azure-hosted targets | Yes (via `az login`) |
...
For `ActiveDirectoryDefault`, run `az login` first; for unattended runs set `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, and `AZURE_CLIENT_SECRET`.
:::
...
For `ActiveDirectoryDefault` and `ActiveDirectoryServicePrincipal`, the `Enter the username` / `Enter the password` prompts are skipped — the identity is resolved at run time (`az login` or `AZURE_*` env vars).
...
| **Username / Password** | Required for `SqlPassword` and `ActiveDirectoryPassword`. Skipped for `ActiveDirectoryDefault` and `ActiveDirectoryServicePrincipal`. | — |
