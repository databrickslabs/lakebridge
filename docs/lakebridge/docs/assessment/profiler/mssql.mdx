...
| `ActiveDirectoryServicePrincipal` | Service Principal — `AZURE_CLIENT_ID` / `AZURE_CLIENT_SECRET` env vars | N/A (non-interactive) |
| `ActiveDirectoryInteractive` | Browser-based interactive login | Yes |

`ActiveDirectoryInteractive` is the right choice when Conditional Access requires MFA. Service Principal is the right
choice for headless / CI runs. On-premises SQL Server (without Entra ID) must use `SqlPassword`.
...
For `ActiveDirectoryServicePrincipal`, the `Enter the username` / `Enter the password` prompts are skipped; set `AZURE_CLIENT_ID` and `AZURE_CLIENT_SECRET` env vars before running the profiler. For `ActiveDirectoryInteractive`, password is never prompted (the driver opens a browser at connect time).
...
| **Username / Password** | Required for `SqlPassword` and `ActiveDirectoryPassword`. Optional pre-fill for `ActiveDirectoryInteractive`. Skipped for `ActiveDirectoryServicePrincipal`. | — |
