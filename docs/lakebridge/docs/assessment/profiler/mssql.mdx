...
| `DefaultAzureCredential` | Entra ID via the driver's `Authentication=ActiveDirectoryDefault` mode — `az login`, SPN env vars, or managed identity. Recommended for Azure-hosted targets | Yes (via `az login`) |
