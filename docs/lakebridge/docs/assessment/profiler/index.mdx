...
3. Store the results in the configured output location as a DuckDB ``.db`` file and a compressed sibling ``.db.zst`` (share the ``.zst`` with Databricks; keep the ``.db`` for local inspection)
