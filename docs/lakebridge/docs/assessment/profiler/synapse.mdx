...
No database driver installation is required: the profiler connects to SQL pools with Microsoft's
...
#### SQL connection to SQL pools

Create or reuse a SQL user with access to the system tables and DMVs listed in [section 3](#3-required-database-permissions), then pick one of the following authentication modes (`configure-database-profiler` prompts for one):

| Auth method | Description | MFA-capable |
|---|---|---|
| `SqlPassword` | SQL Authentication — username + password from credentials file | No |
| `ActiveDirectoryDefault` | Entra ID via the `DefaultAzureCredential` chain — `az login`, SPN env vars, or managed identity. Recommended, and the same chain the management API path uses: one `az login` covers both | Yes (via `az login`) |
...
:::warning
For `ActiveDirectoryServicePrincipal`, set `AZURE_CLIENT_ID` and `AZURE_CLIENT_SECRET` env vars before running the profiler. `AZURE_TENANT_ID` is **not** required for this mode — the driver negotiates the tenant with the server.
For `ActiveDirectoryDefault`, run `az login` first; for unattended runs its environment-variable leg needs all three (`AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`).
:::
