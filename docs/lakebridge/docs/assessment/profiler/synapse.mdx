...
**For local development**: Using `az login` (option 3) is recommended. Ensure your user account has the required roles listed below.

**For automated/CI environments**: Set the three environment variables with a service principal that has the required roles.
