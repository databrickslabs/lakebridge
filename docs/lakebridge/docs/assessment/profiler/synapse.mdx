...
The profiler has two independent auth paths:

1. **Azure Management REST API** (workspace metadata, pool listing, monitoring metrics) uses Azure SDK's `DefaultAzureCredential`, which tries Environment Variables (`AZURE_TENANT_ID` + `AZURE_CLIENT_ID` + `AZURE_CLIENT_SECRET`), Managed Identity, Azure CLI (`az login`), Visual Studio Code, and Azure PowerShell — in that order.
2. **ODBC connection to the SQL pools** uses the auth method you pick from `configure-database-profiler`. See [section 4](#4-odbc-authentication).

`AZURE_TENANT_ID` is only consumed by the management API path; it is **not** needed for ODBC `ActiveDirectoryServicePrincipal`.
...
### 4. ODBC Authentication
Create/Use a user with access to query the following tables in Synapse. The profiler supports the following ODBC `Authentication=` modes (`configure-database-profiler` prompts for one):
...
`ActiveDirectoryInteractive` is the right choice when Conditional Access requires MFA.
