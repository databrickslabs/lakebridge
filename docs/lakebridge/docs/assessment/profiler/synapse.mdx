...
| `DefaultAzureCredential` | Entra ID via the driver's `Authentication=ActiveDirectoryDefault` mode — `az login`, SPN env vars, or managed identity. Recommended, and the same chain the management API path uses: one `az login` covers both | Yes (via `az login`) |
...
For `DefaultAzureCredential`, run `az login` first; for unattended runs its environment-variable leg needs all three (`AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`).
:::
